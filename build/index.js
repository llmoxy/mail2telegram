...
function decodeWord(charset, encoding, str) {
  ...
  return getDecoder3(charset).decode(byteStr);
}
...
var MimeNode = class {
  ...
  setupContentDecoder(transferEncoding) {
    if (/base64/i.test(transferEncoding)) {
      ...
      this.contentDecoder = new QPDecoder({ decoder: getDecoder3(this.contentType.parsed.params.charset) });
    } else {
  ...
  getTextContent() {
    ...
    let str = getDecoder3(this.contentType.parsed.params.charset).decode(this.content);
  ...
  feed(line) {
    switch (this.state) {
      case "header":
        ...
        this.headerLines.push(getDecoder3().decode(line));
        break;
      case "body": {
...
async function sendToWebhook(url, mail, secret, timeoutMs = 5e3) {
  const payload = {
...
async function sendMailToWebhooks(mail, env, deliveredUrls) {
  ...
  const timeoutMs = WEBHOOK_TIMEOUT ? Number.parseInt(WEBHOOK_TIMEOUT, 10) : 5e3;
